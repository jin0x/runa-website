...
@section('content')
  @php
    ...
    $activeCategoryIds = [$currentTerm->term_id];
    $categoryActive = true;
    $isPaged = is_paged();
    $showFeatured = false; // Don't show featured when filtering by category
    ...
    $showFeaturedFilter = false; // Hide featured filter when viewing category
    $showCategoryFilters = !empty($categories) && !$searchActive;
  ...
  {{-- Resources Category Hero Section --}}
  ...
  <x-container :size="ContainerSize::LARGE" classes="py-16">
    ...
    @if (! have_posts())
      <x-alert type="warning" class="mb-8">
        {!! __('Sorry, no resources were found in this category.', 'sage') !!}
      </x-alert>
    @else
      {{-- Resources grid --}}
      @if(have_posts())
